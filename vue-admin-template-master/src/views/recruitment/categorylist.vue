  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column label="操作" width="320" align="center">
        <template slot-scope="scope">
          <router-link :to="'/recruitment/categoryedit/'+scope.row.cid">
            <el-button type="primary" size="mini" icon="el-icon-edit">修改</el-button>
          </router-link>
          <router-link :to="'/recruitment/categorysave'">
  ...
  methods:{
    ...
    selectAllCatego(){
      recrucate.selectAllCategory().then(response=>{
        this.categoryre=response.data.data
        console.log(this.categoryre)
      })
    },
    removeDataById(id){
      recrucate.deleteCategoryById(id).then(response=>{
        this.$message({
          type: 'success',
          message: '删除成功!'
        })
      ...
      this.$router.push({path:'/recruitment/categorysave'})
    }
